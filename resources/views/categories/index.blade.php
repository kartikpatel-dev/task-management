    ...
    <div class="py-12">
        ...
        <div
            ...
            <a href="{{ route('categories.create') }}"
                class="bg-white py-1 px-2 flex justify-end">{{ __('Add Category') }}</a>
        </div>
                ...
                <table width="100%" class="border">
                    <thead>
                        <tr>
                            ...
                            <th class="border p-2">{{ __('Description') }}</th>
                            <th class="border p-2">{{ __('Projects') }}</th>
                        ...
                        @if ($RS_Results->total())
                            @foreach ($RS_Results as $RS_Row)
                                <tr class="border">
                                    ...
                                    <td class="border p-2">{{ $RS_Row->description }}</td>
                                    <td class="border p-2">
                                        @if ($RS_Row->projects->count() > 0)
                                            @foreach ($RS_Row->projects as $RS_Row)
                                                <x-nav-link :href="route('projects.edit', $RS_Row->id)" class="text-gray-900 hover:border-gray-700">
                                                    {{ $RS_Row->name }}
                                                </x-nav-link><br>
                                            @endforeach
                                        @endif
                                    ...
                                    <td class="border p-2">
                                        <center>
                                            <x-primary-button class="mr-2"><a
                                                    href="{{ route('categories.edit', $RS_Row->id) }}"
                                            ...
                                            <x-modal name="confirm-deletion-{{ $RS_Row->id }}" focusable>
                                                <form method="post"
                                                    action="{{ route('categories.destroy', $RS_Row->id) }}"
                                                    class="p-6">
                                                    @csrf
                                                    @method('delete')
